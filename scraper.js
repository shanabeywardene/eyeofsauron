...
/** */
                var topItem = $('div.serp-items div.ui-item:not(.is-top)')[0];
                ...
                data.price = parseFloat($(topItem).find('.item-content').find('.item-info>strong').text().replace('Rs ', '').replace(','));
        ...
        carData : ['formerData', 'urlOfFirstCar', function(next, res) {
            ...
            request(HOST + url, function (err, response, body) {
                ...
                for (var i=0, $i; i<attrArr.length; i++) {
                    $i = $(attrArr[i]);
                    data[$i.find('dt').text().replace(':', '').toLowerCase()] = $i.find('dt').text();
                }
...
function withinTheDay(date) {
    ...
    n_date.setFullYear('2017');
    // console.log((now - n_date)/(1000*60*60*24.0));
    if ( ((now - n_date)/(1000*60*60*24.0)) > 0.5 ) return true;
